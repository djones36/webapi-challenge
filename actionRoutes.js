...
router.get("/:id", validateActionId, (req, res) => {
  ...
  Actions.get(id)
    ...
    .catch(err => {
      res
        .status(500)
        .json({ message: "failed to load action by id", err: err });
    });
});
...
router.delete("/:id", validateActionId, (req, res) => {
  ...
  Actions.remove(id)
    ...
    .catch(err => {
      res.status(500).json({ error: "failure to delete", err: err });
    });
});
...
function validatActions(req, res, next) {
  ...
  if (!action) {
    ...
    res
      .status(404)
      .json({ message: "missing action notes, description, and project_id" });
  } else if (
    action &&
    action.description &&
    action.notes &&
